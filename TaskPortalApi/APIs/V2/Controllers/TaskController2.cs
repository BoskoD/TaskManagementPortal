...
namespace TaskManagementPortal.TaskPortalApi.APIs.V2.Controllers
{
    ...
    public class TaskController2 : ControllerBase
    {
        ...
        [HttpGet("tasks")]
        public async Task<IActionResult> GetAllTasks([FromQuery] PaginationProperties paginationProperties)
        {
            try
            {
                ...
                {
                    _memoryCache.Set("Entities", await _taskRepository.ReadAllASync(paginationProperties));
                }
        ...
        public async Task<IActionResult> GetTaskById(string id)
        {
            try
            {
                var entities = await _taskRepository.GetAll();
        ...
        public async Task<IActionResult> DeleteTask(string id)
        {
            try
            {
                var taskEntity = _taskRepository.GetAll().Result.FirstOrDefault(p => p.RowKey == id);
                if (taskEntity == null)
                {
                    _logger.LogError($"Task with id: {id}, hasn't been found in db.");
                    return NotFound();
